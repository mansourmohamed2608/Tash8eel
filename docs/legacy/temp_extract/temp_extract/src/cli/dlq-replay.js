...
async function listEvents(pool) {
  ...
  `);
    ...
    for (const event of result.rows) {
        ...
        console.log(event.id.padEnd(40) +
...
async function replaySingle(pool, eventId) {
    console.log(`\n🔄 Replaying event ${eventId}...`);
    const result = await pool.query(`SELECT * FROM dlq_events WHERE id = $1`, [eventId]);
    if (result.rows.length === 0) {
...
async function replayForMerchant(pool, merchantId) {
    const result = await pool.query(`
    SELECT * FROM dlq_events 
...
async function replayEvent(pool, event) {
