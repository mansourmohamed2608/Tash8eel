...
abstract class BaseNegotiationPolicy implements INegotiationPolicy {
    ...
    if (catalogItem && catalogItem.minPrice) {
      ...
      if (priceAfterDiscount < catalogItem.minPrice) {
        const maxPossibleDiscount =
          ((catalogItem.basePrice - catalogItem.minPrice) /
  ...
  calculateBundleDiscount(rules: NegotiationRules, cart: Cart): number {
    ...
    const sortedBundles = [...rules.bundleDiscounts].sort(
  ...
  evaluate(
    ...
    if ((request.requestedDiscount || 0) > maxDiscount) {
      return {
        ...
        finalDiscount: 0,
        offerText:
          "للأسف مفيش خصومات على الأكل، لكن ممكن تشوف العروض والكومبوهات! 🍕",
    ...
    return super.evaluate(
      request,
      { ...rules, maxDiscountPercent: maxDiscount },
      cart,
      catalogItem,
    );
  }
...
export class NegotiationPolicyFactory {
  ...
  getPolicy(category: MerchantCategory): INegotiationPolicy {
    return (
      this.policies.get(category) ||
      this.policies.get(MerchantCategory.GENERIC)!
    );
  }
