...
test_1.test.describe("Authentication", () => {
    (0, test_1.test)("login page renders RTL form", async ({ page }) => {
        ...
        await (0, test_1.expect)(page.locator('button[type="submit"]')).toContainText("تسجيل الدخول");
    });
    (0, test_1.test)("wrong credentials shows Arabic error", async ({ page }) => {
        ...
        await (0, test_1.expect)(page.locator(".bg-red-50, .text-red-700").first()).toBeVisible({ timeout: 8_000 });
    });
    (0, test_1.test)("demo merchant logs in and reaches dashboard", async ({ page }) => {
        await (0, auth_1.login)(page, auth_1.DEMO);
        await (0, test_1.expect)(page).toHaveURL(/\/merchant\/dashboard/);
    });
    (0, test_1.test)("admin logs in and reaches admin dashboard", async ({ page }) => {
        await (0, auth_1.login)(page, auth_1.ADMIN);
        await (0, test_1.expect)(page).toHaveURL(/\/admin\/dashboard/);
    });
    (0, test_1.test)("authenticated user is redirected away from /login", async ({ page, }) => {
        // Log in first
        await (0, auth_1.login)(page, auth_1.DEMO);
    ...
    (0, test_1.test)("protected route redirects unauthenticated user to login", async ({ page, }) => {
