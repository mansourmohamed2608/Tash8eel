...
export class EventRepository implements IEventRepository {
  ...
  async findPendingDlqEvents(limit: number): Promise<DlqEvent[]> {
    ...
    return result.rows.map((row: Record<string, unknown>) =>
      this.mapDlqEvent(row),
    );
  }

  async findDlqEventsForRetry(
    before: Date,
    limit: number,
  ): Promise<DlqEvent[]> {
    const result = await this.pool.query(
  ...
  async updateDlqEventStatus(
    id: string,
    status: DlqStatus,
  ): Promise<DlqEvent | null> {
    const result = await this.pool.query(
  ...
  async findReportByDate(
    merchantId: string,
    reportDate: string,
  ): Promise<MerchantReport | null> {
    const result = await this.pool.query(
