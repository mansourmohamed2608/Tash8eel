...
import {
  ...
  PeriodQuerySchema,
  CashRunwayRiskQuerySchema,
  VarianceHealthQuerySchema,
  ReceiveLotSchema,
  MergeSkusSchema,
  FifoCOGSSchema,
  SaveMemorySchema,
...
export class FinanceReportsController {
  ...
  private percentChange(current: number, previous: number): number {
    if (Math.abs(previous) < 0.000001) {
      return Math.abs(current) < 0.000001 ? 0 : 100;
    }
    return this.round2(((current - previous) / Math.abs(previous)) * 100);
  }

  private async getFinanceWindowSnapshot(
    merchantId: string,
    startDate: Date,
    endDate: Date,
  ): Promise<FinanceWindowSnapshot> {
    ...
    const result = await this.pool.query(
      `WITH order_stats AS (
         SELECT
           ...
           COALESCE(SUM(
             CASE
               WHEN o.status::text NOT IN ('CANCELLED', 'DRAFT', 'FAILED', 'RETURNED')
               THEN ${orderAmountExpr}
               ELSE 0
             END
           ), 0) AS booked_sales,
           ...
           COALESCE(SUM(
             CASE
               WHEN o.status::text NOT IN ('CANCELLED', 'DRAFT', 'FAILED', 'RETURNED')
               THEN GREATEST(${orderAmountExpr} - ${paidOrderAmountExpr}, 0)
               ELSE 0
             END
       ...
       refund_stats AS (
         SELECT
           COALESCE(SUM(amount) FILTER (WHERE status = 'APPROVED'), 0) AS refunds_amount,
           COUNT(*) FILTER (WHERE status = 'APPROVED') AS approved_refunds_count
         FROM refunds
         WHERE merchant_id = $1
       ...
       SELECT
         os.order_count,
         os.booked_sales,
         os.collected_revenue,
         os.pending_collections,
         es.total_expenses,
         rs.refunds_amount,
         rs.approved_refunds_count
       FROM order_stats os
    ...
    return {
      orderCount: this.parseInteger(row.order_count),
      bookedSales: this.round2(this.parseNumeric(row.booked_sales)),
      collectedRevenue: this.round2(this.parseNumeric(row.collected_revenue)),
      pendingCollections: this.round2(
        this.parseNumeric(row.pending_collections),
      ),
      totalExpenses: this.round2(this.parseNumeric(row.total_expenses)),
      refundsAmount: this.round2(this.parseNumeric(row.refunds_amount)),
      approvedRefundsCount: this.parseInteger(row.approved_refunds_count),
    };
  }
  ...
  @Get(":merchantId/cash-runway-risk")
  @ApiOperation({
    summary:
      "Deterministic cash runway and collections risk from orders, payments, and expenses",
  })
  @ApiQuery({
    name: "periodDays",
    required: false,
    description: "Lookback window in days (7-180)",
  })
  @ApiQuery({
    name: "cashReserve",
    required: false,
    description: "Current available cash reserve",
  })
  @ApiQuery({
    name: "overdueDays",
    required: false,
    description: "Receivable aging threshold for overdue risk",
  })
  async getCashRunwayRisk(
    @Param("merchantId") merchantId: string,
    @Query(new ZodValidationPipe(CashRunwayRiskQuerySchema))
    query: CashRunwayRiskQueryDto,
  ) {
    const periodDays = query.periodDays ?? 30;
    const cashReserve = query.cashReserve ?? 0;
    ...
    const snapshot = await this.getFinanceWindowSnapshot(
      merchantId,
      startDate,
      endDate,
    );
    ...
    const orderAmountExpr = this.orderAmountExpr("o");
    const paidOrderAmountExpr = this.paidOrderAmountExpr("o");
    const overdueCutoff = new Date(endDate);
    overdueCutoff.setUTCDate(overdueCutoff.getUTCDate() - overdueDays);

    const outstandingResult = await this.pool.query(
      `WITH unpaid_orders AS (
         SELECT
           o.id,
           o.created_at,
           GREATEST(${orderAmountExpr} - ${paidOrderAmountExpr}, 0) AS outstanding_amount
         FROM orders o
         LEFT JOIN (
           SELECT
             order_id,
       ...
       SELECT
         COUNT(*) FILTER (WHERE outstanding_amount > 0) AS unpaid_orders_count,
         COUNT(*) FILTER (
           WHERE outstanding_amount > 0
             AND created_at < $4::timestamptz
         ) AS overdue_unpaid_orders_count,
         COALESCE(SUM(
           CASE WHEN outstanding_amount > 0 THEN outstanding_amount ELSE 0 END
         ), 0) AS total_outstanding,
         COALESCE(SUM(
           CASE
             WHEN outstanding_amount > 0 AND created_at < $4::timestamptz
      ...
      [
        ...
        endDate.toISOString(),
        overdueCutoff.toISOString(),
      ],
    );

    const outstandingRow =
      (outstandingResult.rows[0] as Record<string, unknown>) || {};
    const totalOutstanding = this.round2(
      this.parseNumeric(outstandingRow.total_outstanding),
    );
    const overdueOutstanding = this.round2(
      this.parseNumeric(outstandingRow.overdue_outstanding),
    );
    const unpaidOrdersCount = this.parseInteger(
      outstandingRow.unpaid_orders_count,
    );
    const overdueUnpaidOrdersCount = this.parseInteger(
      outstandingRow.overdue_unpaid_orders_count,
    );

    const avgDailyCollected = this.round2(
      snapshot.collectedRevenue / periodDays,
    );
    const avgDailyExpenses = this.round2(snapshot.totalExpenses / periodDays);
    const avgDailyNetCashFlow = this.round2(
      avgDailyCollected - avgDailyExpenses,
    );
    const netDailyBurn = this.round2(
      Math.max(avgDailyExpenses - avgDailyCollected, 0),
    );
    const runwayDays =
      netDailyBurn > 0 ? this.round2(cashReserve / netDailyBurn) : null;

    const pendingToSalesRatio =
      snapshot.bookedSales > 0 ? totalOutstanding / snapshot.bookedSales : 0;
    const overdueToOutstandingRatio =
      totalOutstanding > 0 ? overdueOutstanding / totalOutstanding : 0;
    const coverageRatio =
      avgDailyExpenses > 0
        ? this.round2(avgDailyCollected / avgDailyExpenses)
        : avgDailyCollected > 0
          ? 999
          : 1;

    const criticalRunway = runwayDays !== null && runwayDays < 10;
    const shortRunway = runwayDays !== null && runwayDays < 21;
    const heavyCollectionGap = pendingToSalesRatio >= 0.25;
    const highOverduePressure = overdueToOutstandingRatio >= 0.5;

    let riskLevel: "LOW" | "MEDIUM" | "HIGH" | "CRITICAL" = "LOW";
    if (criticalRunway || (heavyCollectionGap && highOverduePressure)) {
      riskLevel = "CRITICAL";
    } else if (shortRunway || (heavyCollectionGap && avgDailyNetCashFlow < 0)) {
      riskLevel = "HIGH";
    } else if (
    ...
    return {
      ...
      cashReserve: this.round2(cashReserve),
      operatingMode:
        avgDailyNetCashFlow >= 0 ? "CASH_POSITIVE" : "BURNING_CASH",
      riskLevel,
      runway: {
        avgDailyCollected,
        avgDailyExpenses,
        ...
        netDailyBurn,
        runwayDays,
        revenueExpenseCoverageRatio: coverageRatio,
      },
      collections: {
        bookedSales: snapshot.bookedSales,
        collectedRevenue: snapshot.collectedRevenue,
        pendingCollections: totalOutstanding,
        overdueCollections: overdueOutstanding,
        unpaidOrdersCount,
        overdueUnpaidOrdersCount,
        pendingCollectionsPctOfSales: this.round2(pendingToSalesRatio * 100),
        overduePctOfPending: this.round2(overdueToOutstandingRatio * 100),
        overdueThresholdDays: overdueDays,
      },
      totals: {
        orders: snapshot.orderCount,
        totalExpenses: snapshot.totalExpenses,
        approvedRefundsCount: snapshot.approvedRefundsCount,
        refundsAmount: snapshot.refundsAmount,
      },
      signals: {
        shortRunway,
        heavyCollectionGap,
        highOverduePressure,
        negativeDailyCashFlow: avgDailyNetCashFlow < 0,
      },
    };
  }

  @Get(":merchantId/variance-health")
  @ApiOperation({
    summary:
      "Current-vs-previous finance variance and operational health score",
  })
  @ApiQuery({
    name: "periodDays",
    required: false,
    description: "Period length in days for both current and previous windows",
  })
  async getVarianceHealth(
    @Param("merchantId") merchantId: string,
    @Query(new ZodValidationPipe(VarianceHealthQuerySchema))
    query: VarianceHealthQueryDto,
  ) {
    const periodDays = query.periodDays ?? 30;
    ...
    const current = await this.getFinanceWindowSnapshot(
      merchantId,
      currentStart,
      currentEnd,
    );
    const previous = await this.getFinanceWindowSnapshot(
      merchantId,
      previousStart,
      previousEnd,
    );

    const currentNetCashFlow = this.round2(
      current.collectedRevenue - current.totalExpenses,
    );
    const previousNetCashFlow = this.round2(
      previous.collectedRevenue - previous.totalExpenses,
    );

    const collectionEfficiency =
      current.bookedSales > 0
        ? current.collectedRevenue / current.bookedSales
        : 1;
    const pendingCollectionsRatio =
      current.bookedSales > 0
        ? current.pendingCollections / current.bookedSales
        : 0;
    const expenseLoadRatio =
      current.collectedRevenue > 0
        ? current.totalExpenses / current.collectedRevenue
        : current.totalExpenses > 0
          ? 2
          : 0;
    const refundPressureRatio =
      current.collectedRevenue > 0
        ? current.refundsAmount / current.collectedRevenue
        : current.refundsAmount > 0
          ? 1
          : 0;

    let healthScore = 100;
    healthScore -= Math.min(30, pendingCollectionsRatio * 120);
    healthScore -= Math.min(25, Math.max(0, expenseLoadRatio - 0.85) * 100);
    healthScore -= Math.min(15, Math.max(0, 0.92 - collectionEfficiency) * 120);
    healthScore -= Math.min(15, refundPressureRatio * 200);
    if (currentNetCashFlow < 0) {
      healthScore -= Math.min(
        25,
        (Math.abs(currentNetCashFlow) / Math.max(current.collectedRevenue, 1)) *
          100,
      );
    }
    ...
    return {
      periodDays,
      currentPeriod: {
        startDate: this.toDateOnly(currentStart),
        endDate: this.toDateOnly(currentEnd),
        orderCount: current.orderCount,
        bookedSales: current.bookedSales,
        collectedRevenue: current.collectedRevenue,
        pendingCollections: current.pendingCollections,
        totalExpenses: current.totalExpenses,
        refundsAmount: current.refundsAmount,
        netCashFlow: currentNetCashFlow,
      },
      previousPeriod: {
        startDate: this.toDateOnly(previousStart),
        endDate: this.toDateOnly(previousEnd),
        orderCount: previous.orderCount,
        bookedSales: previous.bookedSales,
        collectedRevenue: previous.collectedRevenue,
        pendingCollections: previous.pendingCollections,
        totalExpenses: previous.totalExpenses,
        refundsAmount: previous.refundsAmount,
        netCashFlow: previousNetCashFlow,
      },
      variance: {
        bookedSalesPct: this.percentChange(
          current.bookedSales,
          previous.bookedSales,
        ),
        collectedRevenuePct: this.percentChange(
          current.collectedRevenue,
          previous.collectedRevenue,
        ),
        pendingCollectionsPct: this.percentChange(
          current.pendingCollections,
          previous.pendingCollections,
        ),
        totalExpensesPct: this.percentChange(
          current.totalExpenses,
          previous.totalExpenses,
        ),
        refundsAmountPct: this.percentChange(
          current.refundsAmount,
          previous.refundsAmount,
        ),
        netCashFlowPct: this.percentChange(
          currentNetCashFlow,
          previousNetCashFlow,
        ),
      },
      health: {
        score: healthScore,
        band: healthBand,
        collectionEfficiencyPct: this.round2(collectionEfficiency * 100),
        pendingCollectionsPctOfSales: this.round2(
          pendingCollectionsRatio * 100,
        ),
        expenseLoadPctOfCollected: this.round2(expenseLoadRatio * 100),
        refundPressurePctOfCollected: this.round2(refundPressureRatio * 100),
        signals: {
          weakCollections: collectionEfficiency < 0.85,
          highPendingCollections: pendingCollectionsRatio >= 0.25,
          expensePressure: expenseLoadRatio > 1,
          negativeNetCashFlow: currentNetCashFlow < 0,
          elevatedRefundPressure: refundPressureRatio > 0.1,
        },
      },
    };
  }
