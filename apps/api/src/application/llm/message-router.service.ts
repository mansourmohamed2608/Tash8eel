...
export class MessageRouterService {
  ...
  async getInstantReply(
    ...
    if (rawText.length < 20 && this.greetingPhrases.has(normalized)) {
      const welcomeMessage = await this.getWelcomeMessage(merchantId);
    ...
    if (
      rawText.length < 50 &&
      this.containsAny(normalized, this.priceKeywords)
    ) {
      const priceReply = await this.getCatalogPriceReply(merchantId, rawText);
      if (priceReply) {
        return priceReply;
      }
    }
