...
describe("PlannerOrchestrationService", () => {
  ...
  it("blocks deterministic boundary actions when triggered by scheduler", async () => {
    const plannerContextAssembler = {
      assemble: jest.fn().mockResolvedValue({
        ...baseContext,
      }),
    ...
    expect(decision.allowed).toBe(false);
    expect(decision.reasons).toContain(
      "Deterministic boundary action UPDATE_STOCK requires on-demand or escalation trigger, received SCHEDULED",
    );
  });

  it("blocks always-on loop trigger sources", async () => {
    const plannerContextAssembler = {
      assemble: jest.fn().mockResolvedValue({
        ...baseContext,
      }),
  ...
  it("applies deterministic governance fallback when budget check is unavailable", async () => {
    const plannerContextAssembler = {
      assemble: jest.fn().mockResolvedValue({
        ...baseContext,
      }),
    ...
    const controlPlaneGovernance = {
      checkTriggerBudget: jest.fn().mockRejectedValue(new Error("ledger down")),
      recordPlannerRun: jest.fn().mockResolvedValue({ runId: "run-2" }),
    } as any;
  ...
  it("keeps read-only execution available when governance fallback is active", async () => {
    const plannerContextAssembler = {
      assemble: jest.fn().mockResolvedValue({
        ...baseContext,
      }),
    ...
    const controlPlaneGovernance = {
      checkTriggerBudget: jest.fn().mockRejectedValue(new Error("ledger down")),
      recordPlannerRun: jest.fn().mockResolvedValue({ runId: "run-3" }),
    } as any;
