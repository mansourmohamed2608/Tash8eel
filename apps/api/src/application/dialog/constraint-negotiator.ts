...
export interface ConstraintNegotiationPlan {
  axes: string[];
  axisLabelsAr: string[];
  reasoning: string;
}
...
export class ConstraintNegotiator {
  static plan(
    ...
    merchant: Merchant,
  ): ConstraintNegotiationPlan | null {
    ...
    return {
      axes,
      axisLabelsAr: axes.map((axis) => AXIS_LABELS[axis] || axis),
  ...
  static compose(
    ...
    const plan = this.plan(customerMessage, merchant);
    if (!plan) return null;

    const labels = plan.axisLabelsAr;
  ...
  private static detectAxes(text: string, merchant: Merchant): string[] {
    ...
    if (this.hasPersonalizationRequest(normalized)) add("personalization");
  ...
  private static hasPersonalizationRequest(text: string): boolean {
    return /حفر|نقش|كتابة\s*اسم|اكتب\s*اسم|تخصيص|مخصص|personaliz|engraving|engrave/i.test(
      text,
    );
  }
}
