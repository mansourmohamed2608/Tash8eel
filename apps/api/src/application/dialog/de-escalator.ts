...
export interface DeEscalationPlan {
  needsEmotionAck: boolean;
  hasComplaint: boolean;
  hasHumanDemand: boolean;
  backupMode: string;
  reasoning: string;
...
export class DeEscalator {
  static plan(customerMessage: string, merchant: Merchant): DeEscalationPlan {
    ...
    const backupMode = String(agentAvailability.backup || "none").toLowerCase();
    ...
    return {
      needsEmotionAck: hasComplaint || hasHumanDemand,
      hasComplaint,
      hasHumanDemand,
      backupMode,
      reasoning: "dialog_deescalator:structured_plan",
      forbiddenClaims:
        backupMode === "none"
          ? [
              "do not promise transfer",
              "do not say someone else will contact the customer",
