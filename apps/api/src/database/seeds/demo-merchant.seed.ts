...
async function seed(): Promise<void> {
  ...
  try {
    ...
    const products: Product[] = [
      ...
      {
        sku: "TBL-001",
        name_ar: "مفرش سفرة مطرز 6 أشخاص",
        price: 280,
        stock: 12,
        threshold: 4,
        supplierId: supplier1Id,
      ...
      {
        sku: "DSH-001",
        name_ar: "طقم أطباق بورسلين 24 قطعة",
        price: 420,
        stock: 14,
        threshold: 4,
        supplierId: supplier1Id,
    ...
    console.log(
      `✅ STEP 5 — 8 customers created (totals left at 0 for Step 7)\n`,
    );
    ...
    const orderDefs: OrderDef[] = [
      ...
      // ORD-004 — محمد — DRAFT (pending) — unpaid — today
      {
        ...
        date: now,
        items: [{ sku: "RUG-001", qty: 1, price: 850 }],
      },
      ...
      // ORD-006 — سارة — CONFIRMED — unpaid — 2 days ago ⚠️ TRIGGERS COLLECTION
      {
        ...
        paymentStatus: "unpaid",
        date: daysAgo(2),
        items: [{ sku: "CRT-001", qty: 1, price: 680 }],
      },
      // ORD-007 — أحمد — DELIVERED — paid — 26 days ago ⚠️ TRIGGERS WIN-BACK
      {
        ...
        paymentStatus: "paid",
        date: daysAgo(26),
        items: [{ sku: "SHT-001", qty: 1, price: 450 }],
      },
      ...
      // ORD-010 — كريم — DELIVERED — paid — 20 days ago
      {
        ...
        paymentStatus: "paid",
        date: daysAgo(20),
        items: [{ sku: "TWL-001", qty: 2, price: 320 }],
      },
      // ORD-011 — كريم — CONFIRMED — paid — today
      {
        ...
        paymentStatus: "paid",
        date: now,
        items: [{ sku: "DSH-001", qty: 1, price: 420 }],
      },
      ...
      // ORD-015 — هبة — DELIVERED — paid — 3 days ago
      {
        ...
        paymentStatus: "paid",
        date: daysAgo(3),
        items: [{ sku: "CVR-001", qty: 2, price: 390 }],
      },
    ];
    ...
    for (const ord of orderDefs) {
      ...
      const customerName = customerDefs.find(
        (c) => c.key === ord.customerKey,
      )!.name;
      const customerPhone = customerDefs.find(
        (c) => c.key === ord.customerKey,
      )!.phone;
    ...
    console.log(
      `✅ STEP 6 — 15 orders + order_items created, last_order_at updated\n`,
    );
    ...
    console.log(
      `✅ STEP 7 — Customer totals, stock quantities, finance snapshots recalculated\n`,
    );
    ...
    console.log(
      `✅ STEP 8 — Stock alerts created (low_stock: TWL-001, out_of_stock: PIL-001)\n`,
    );
    ...
    const convDefs: ConvDef[] = [
      {
        ...
        messages: [
          { direction: "inbound", content: "عايز أطلب طقم شراشف", isAi: false },
          {
            direction: "outbound",
            content:
              "أهلاً محمد! طقم الشراشف القطن المزدوج متاح بسعر 450 جنيه. تأكد الطلب؟",
            isAi: true,
          },
          { direction: "inbound", content: "آه كويس", isAi: false },
          {
            direction: "outbound",
            content:
              "تم تأكيد طلبك! رقم الطلب #ORD-003. سيتم التواصل معك قريباً للتوصيل.",
            isAi: true,
          },
        ],
      },
      {
        ...
        messages: [
          ...
          {
            direction: "outbound",
            content: "طلبك رقم #ORD-006 قيد التأكيد. في انتظار إتمام الدفع.",
            isAi: true,
          },
        ],
      },
      {
        ...
        messages: [
          { direction: "inbound", content: "كام سعر الستاير؟", isAi: false },
          {
            direction: "outbound",
            content:
              "أهلاً يوسف! ستائر غرفة النوم بسعر 680 جنيه للطقم الكامل. تحب تطلب؟",
            isAi: true,
          },
        ],
      },
    ...
    console.log(
      `   Cleanup          : DELETE FROM merchants WHERE id = '${merchantId}';\n`,
    );
    console.log("━".repeat(60));
    ...
    console.log(
      `V1 — Order total mismatches (MUST be 0 rows): ${v1.rowCount} rows`,
    );
    if (v1.rowCount! > 0) {
      console.log("  ⚠️  MISMATCHES FOUND:");
      console.table(v1.rows);
    } else {
      console.log("  ✅ PASS — all order totals match order_items sums");
    }
    ...
    if (v3.rowCount! > 0) {
      ...
      console.log(
        "  ✅ PASS — catalog_items.stock_quantity matches inventory_variants",
      );
    }
