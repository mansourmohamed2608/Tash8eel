...
describe("Conversations Endpoints (e2e)", () => {
  ...
  describe("GET /api/v1/conversations/:id", () => {
    it("should return conversation details", async () => {
      const response = await request(app.getHttpServer())
        .get(
          `/api/v1/conversations/${testConversationId}?merchantId=${testMerchantId}`,
        )
    ...
    it("should include messages when requested", async () => {
      const response = await request(app.getHttpServer())
        .get(
          `/api/v1/conversations/${testConversationId}?merchantId=${testMerchantId}&includeMessages=true`,
        )
        .set("x-admin-api-key", adminApiKey)
        .expect(200);
    ...
    it("should return 404 for non-existent conversation", async () => {
      await request(app.getHttpServer())
        .get(
          `/api/v1/conversations/00000000-0000-0000-0000-000000000000?merchantId=${testMerchantId}`,
        )
        .set("x-admin-api-key", adminApiKey)
        .expect(404);
    });
  ...
  describe("POST /api/v1/conversations/:id/takeover", () => {
    ...
    it("should initiate human takeover", async () => {
      ...
      const response = await request(app.getHttpServer())
        .post(
          `/api/v1/conversations/${testConversationId}/takeover?merchantId=${testMerchantId}`,
        )
        .set("x-admin-api-key", adminApiKey)
        .send({
  ...
  describe("POST /api/v1/conversations/:id/release", () => {
    ...
    it("should release conversation back to AI", async () => {
      ...
      const response = await request(app.getHttpServer())
        .post(
          `/api/v1/conversations/${testConversationId}/release?merchantId=${testMerchantId}`,
        )
        .set("x-admin-api-key", adminApiKey)
        .send({
          operatorId: "operator-001",
  ...
  describe("GET /api/v1/conversations", () => {
    ...
    it("should filter by state", async () => {
      const response = await request(app.getHttpServer())
        .get(
          `/api/v1/conversations?merchantId=${testMerchantId}&state=COLLECTING_ITEMS`,
        )
        .set("x-admin-api-key", adminApiKey)
        .expect(200);
    ...
    it("should support pagination", async () => {
      const response = await request(app.getHttpServer())
        .get(
          `/api/v1/conversations?merchantId=${testMerchantId}&page=1&limit=5`,
        )
        .set("x-admin-api-key", adminApiKey)
        .expect(200);
