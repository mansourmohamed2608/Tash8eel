...
describe("Twilio WhatsApp Webhook (e2e)", () => {
    ...
    beforeAll(async () => {
        ...
        twilioAuthToken =
            configService.get("TWILIO_AUTH_TOKEN") || "test_auth_token";
        validateSignature =
            configService.get("TWILIO_VALIDATE_SIGNATURE") !== "false";
    ...
    afterAll(async () => {
        ...
        await pool.query("DELETE FROM twilio_message_log WHERE from_number = $1 OR to_number = $1", [merchantWhatsApp]);
        await pool.query("DELETE FROM messages WHERE merchant_id = $1", [
            merchantId,
        ]);
        await pool.query("DELETE FROM order_items WHERE order_id IN (SELECT id FROM orders WHERE merchant_id = $1)", [merchantId]);
        ...
        await pool.query("DELETE FROM merchant_phone_numbers WHERE merchant_id = $1", [merchantId]);
     ...
     */
    function createTwilioSignature(url, params) {
    ...
    describe("POST /api/v1/webhooks/twilio/whatsapp", () => {
        describe("Basic Text Messages", () => {
            ...
            it("should process an order request", async () => {
                ...
                const { rows: conversations } = await pool.query("SELECT * FROM conversations WHERE merchant_id = $1 AND sender_id = $2 ORDER BY created_at DESC LIMIT 1", [merchantId, "+201009876543"]);
    ...
    describe("POST /api/v1/webhooks/twilio/status", () => {
      ...
      `, [messageSid, merchantWhatsApp, customerWhatsApp]);
            ...
            const { rows } = await pool.query("SELECT status FROM twilio_message_log WHERE message_sid = $1", [messageSid]);
            if (rows.length > 0) {
    ...
    describe("Integration Flow: Complete Order via WhatsApp", () => {
      ...
      `, [merchantId, orderCustomerPhone.replace("whatsapp:", "")]);
            ...
            expect(parseInt(conversations[0].message_count, 10)).toBeGreaterThanOrEqual(3);
        });
    });
    describe("Edge Cases", () => {
    ...
    describe("Customer Reorder Flow", () => {
        ...
        let reorderCustomerId;
        let reorderCatalogItemId;
        beforeAll(async () => {
      ...
      `, [
        ...
        it('should detect "نفس الطلب" and show previous order summary', async () => {
            ...
            const { rows: conversations } = await pool.query(`SELECT * FROM conversations 
         WHERE merchant_id = $1 AND sender_id = $2 
         ORDER BY created_at DESC LIMIT 1`, [merchantId, reorderCustomerPhone]);
            expect(conversations.length).toBe(1);
        ...
        it('should complete reorder when customer confirms with "تمام"', async () => {
            ...
            const confirmResponse = await (0, supertest_1.default)(app.getHttpServer())
                .post("/api/v1/webhooks/twilio/whatsapp")
                .set("Content-Type", "application/x-www-form-urlencoded")
                .set("x-twilio-signature", "test-sig")
                .send(confirmPayload)
         ...
         ORDER BY created_at DESC LIMIT 1`, [merchantId, reorderCustomerId]);
        ...
        it("should handle reorder when items are out of stock", async () => {
            ...
            await pool.query("UPDATE inventory SET quantity = 0 WHERE catalog_item_id = $1", [reorderCatalogItemId]);
            const payload = createWebhookPayload({
                Body: "نفس الطلب",
                From: reorderCustomerWhatsApp,
            ...
            expect(response.text).toContain("<Response>");
            // Restore inventory
            await pool.query("UPDATE inventory SET quantity = 50 WHERE catalog_item_id = $1", [reorderCatalogItemId]);
        });
    });
});
