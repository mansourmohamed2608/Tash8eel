...
if (!process.env.DATABASE_URL) {
    console.error("❌ DATABASE_URL not set. Tests require a database connection.");
    console.error("Set DATABASE_URL environment variable or create .env file in project root.");
...
console.log(`[jest.setup] Using DATABASE_URL: ${process.env.DATABASE_URL?.replace(/:[^:@]+@/, ":****@")}`);
