...
describe("Payment Method Classification", () => {
  ...
  describe("InstaPay Detection", () => {
    ...
    it('should detect "ipa@alias" reference', () => {
      expect(normalizePaymentMethod("payment via ipa@merchant")).toBe(
        "INSTAPAY",
      );
    });
  });
  ...
  describe("Unknown Detection", () => {
    ...
    it("should return UNKNOWN for null/undefined", () => {
      expect(normalizePaymentMethod(null as any)).toBe("UNKNOWN");
      expect(normalizePaymentMethod(undefined as any)).toBe("UNKNOWN");
    });
  ...
  describe("Payment Proof Auto-Verification Logic", () => {
    ...
    function shouldAutoVerify(input: ProofVerificationInput): boolean {
      const {
        extractedAmount,
        expectedAmount,
        paymentMethod,
