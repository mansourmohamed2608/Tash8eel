...
import { NegotiationRules, Cart } from "../../src/shared/schemas";
import { NegotiationRequest } from "../../src/domain/policies/negotiation-policy.interface";

/**
 * Helper to create a NegotiationRequest from discount percentage
 */
function createRequest(requestedDiscount: number): NegotiationRequest {
...
function createRules(
  overrides: Partial<NegotiationRules> = {},
): NegotiationRules {
  return {
 ...
 */
function createCart(itemCount: number, quantityEach = 1): Cart {
...
describe("NegotiationPolicyFactory", () => {
  ...
  describe("SupermarketNegotiationPolicy", () => {
    const policy = NegotiationPolicyFactory.create(
