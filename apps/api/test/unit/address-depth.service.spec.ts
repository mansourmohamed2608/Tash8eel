import {
  ...
  AddressDepth,
  GoogleMapsLocation,
} from "../../src/application/services/address-depth.service";
import {
...
describe("AddressDepthService", () => {
  ...
  let factory: AddressValidationPolicyFactory;

  beforeEach(() => {
    const defaultValidator = new DefaultAddressValidator();
    const cairoValidator = new CairoAddressValidator();

    factory = new AddressValidationPolicyFactory(
      cairoValidator,
      {} as any, // GizaValidator
      {} as any, // AlexandriaValidator
      defaultValidator,
    );

    service = new AddressDepthService(factory);
  });
  ...
  describe("extractLocationFromText", () => {
    it("should extract Google Maps URL from text", () => {
      const text =
        "عنواني هو https://www.google.com/maps/@30.0444,31.2357,17z بجوار المول";
