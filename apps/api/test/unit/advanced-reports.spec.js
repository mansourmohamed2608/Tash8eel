...
const advanced_reports_controller_1 = require("../../src/api/controllers/advanced-reports.controller");
// ============================================================================
// MOCK POOL FACTORY
// ============================================================================
...
describe("FinanceReportsController", () => {
    ...
    beforeEach(() => {
        pool = createMockPool();
        controller = new advanced_reports_controller_1.FinanceReportsController(pool);
    });
    ...
    describe("GET /:merchantId/cash-flow-forecast", () => {
        it("should forecast cash flow for 30 days", async () => {
            ...
            expect(["LOW", "MEDIUM", "HIGH"]).toContain(result.summary.confidenceLevel);
        });
...
describe("AdvancedInventoryController", () => {
    ...
    beforeEach(() => {
        pool = createMockPool();
        controller = new advanced_reports_controller_1.AdvancedInventoryController(pool);
    });
    ...
    describe("POST /:merchantId/lots", () => {
        ...
        it("should rollback on error", async () => {
            ...
            await expect(controller.receiveLot("merchant-1", {
                itemId: "item-1",
                lotNumber: "LOT-FAIL",
                quantity: 10,
                costPrice: 5,
            ...
            const rollbackCalls = client.query.mock.calls.filter((c) => typeof c[0] === "string" && c[0].includes("ROLLBACK"));
    ...
    describe("POST /:merchantId/merge-skus", () => {
        it("should reject same source and target", async () => {
            await expect(controller.mergeSkus("merchant-1", {
                sourceItemId: "item-1",
                targetItemId: "item-1",
...
describe("CustomerIntelligenceController", () => {
    ...
    beforeEach(() => {
        pool = createMockPool();
        controller = new advanced_reports_controller_1.CustomerIntelligenceController(pool);
    });
    ...
    describe("GET /:merchantId/ai-decisions", () => {
        it("should filter by agent and decision type", async () => {
            ...
            const result = await controller.getAiDecisionLog("merchant-1", "OPS_AGENT", "UPSELL");
        ...
        it("should limit results to 200 max", async () => {
            pool.query.mockResolvedValue({ rows: [] });
            await controller.getAiDecisionLog("merchant-1", undefined, undefined, undefined, undefined, "500");
