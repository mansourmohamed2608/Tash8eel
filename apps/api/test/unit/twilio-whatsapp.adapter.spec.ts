...
import {
  ...
  MockTwilioWhatsAppAdapter,
  TwilioWebhookPayload,
  TwilioStatusPayload,
} from "../../src/application/adapters/twilio-whatsapp.adapter";

describe("TwilioWhatsAppAdapter", () => {
  describe("MockTwilioWhatsAppAdapter", () => {
    let adapter: MockTwilioWhatsAppAdapter;

    beforeEach(() => {
      adapter = new MockTwilioWhatsAppAdapter();
    });

    describe("validateSignature", () => {
      it("should always return true in mock mode", () => {
        expect(
          adapter.validateSignature("any-signature", "http://example.com", {}),
        ).toBe(true);
      });
    });
    ...
    describe("parseWebhook", () => {
      ...
      it("should parse message with image attachment (not voice note)", () => {
        ...
        expect(result.hasMedia).toBe(true);
        expect(result.isVoiceNote).toBe(false);
        expect(result.mediaUrls).toContain(
          "https://api.twilio.com/media/image.jpg",
        );
      });
    });
    ...
    describe("getMerchantByWhatsAppNumber", () => {
      it("should return mock merchant", async () => {
        const result = await adapter.getMerchantByWhatsAppNumber(
    ...
    describe("downloadMedia", () => {
      it("should return mock audio buffer", async () => {
        const result = await adapter.downloadMedia(
          "https://api.twilio.com/media/audio.ogg",
        );
