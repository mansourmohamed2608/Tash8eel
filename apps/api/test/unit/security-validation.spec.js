...
describe("Scope 1 — Staff Auth Guards", () => {
    ...
    describe("change-password guard isolation", () => {
        it("JWT bearer path sets staffId on request", () => {
            ...
            const token = jwt.sign({ staffId: "staff-001", merchantId: "merch-001", role: "ADMIN" }, SECRET, { expiresIn: "1h" });
    ...
    describe("logout IDOR protection", () => {
        it("derives staffId from verifyRefreshTokenPayload, not from body", () => {
            ...
            const refreshToken = jwt.sign({ staffId: "staff-real", merchantId: "merch-001" }, SECRET, { expiresIn: "7d" });
...
describe("Scope 2 — Public Payment Routes", () => {
    ...
    describe("SSRF prevention on imageUrl", () => {
        ...
        it("blocks localhost", () => {
            expect(validateUrl("http://localhost/image.png")).toBe("Invalid image URL.");
        });
        it("blocks 127.0.0.1", () => {
            expect(validateUrl("http://127.0.0.1/img.png")).toBe("Invalid image URL.");
        });
        ...
        it("blocks 192.168.x private range", () => {
            expect(validateUrl("http://192.168.1.1/img.png")).toBe("Invalid image URL.");
        });
        it("blocks link-local 169.254.x", () => {
            expect(validateUrl("http://169.254.169.254/latest/meta-data/")).toBe("Invalid image URL.");
        });
        it("blocks ftp:// protocol", () => {
            expect(validateUrl("ftp://example.com/img.png")).toBe("Invalid image URL protocol.");
        });
        it("blocks file:// protocol", () => {
            expect(validateUrl("file:///etc/passwd")).toBe("Invalid image URL protocol.");
        });
    ...
    describe("payment link URL generation", () => {
        ...
        it("portal pay page route matches API-generated URL path", () => {
            ...
            expect(apiUrlTemplate.replace("${linkCode}", "PAY-TEST")).toBe("/pay/PAY-TEST");
        });
    });
});
// =============================================================================
...
describe("Scope 3 — Rate Limiting", () => {
    describe("rate limit configuration values", () => {
        ...
        it("forgot-password has the strictest limit", () => {
            const sorted = Object.values(CONFIGURED_LIMITS).sort((a, b) => a.limit - b.limit);
...
describe("Scope 4 — Pricing & Entitlements Config", () => {
    ...
    describe("FE/BE price consistency", () => {
        it("STARTER price matches: 299 EGP", () => {
            ...
            expect(FRONTEND_PLANS.STARTER.currency).toBe(BACKEND_PLANS.STARTER.currency);
        });
    ...
    describe("entitlement enforcement", () => {
        it("EntitlementGuard reads enabled_agents from merchants DB table", () => {
            ...
            const query = "SELECT enabled_agents, enabled_features FROM merchants WHERE id = $1 AND is_active = true";
