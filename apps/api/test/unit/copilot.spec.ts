...
describe("Copilot Schema", () => {
  ...
  describe("Intent Feature Mapping", () => {
    it("should map ADD_EXPENSE to REPORTS feature", () => {
      expect(INTENT_FEATURE_MAP["ADD_EXPENSE"]).toContain("REPORTS");
    });

    it("should map UPDATE_STOCK to INVENTORY feature", () => {
      expect(INTENT_FEATURE_MAP["UPDATE_STOCK"]).toContain("INVENTORY");
    });

    it("should map ASK_KPI to KPI_DASHBOARD feature", () => {
      expect(INTENT_FEATURE_MAP["ASK_KPI"]).toContain("KPI_DASHBOARD");
    });

    it("should map CREATE_PAYMENT_LINK to PAYMENTS feature", () => {
      expect(INTENT_FEATURE_MAP["CREATE_PAYMENT_LINK"]).toContain("PAYMENTS");
    });

    it("should have feature mapping for most intents", () => {
      const mappedIntents = Object.keys(INTENT_FEATURE_MAP);
  ...
  describe("Destructive Intents", () => {
    it("should include ADD_EXPENSE as destructive", () => {
      expect(DESTRUCTIVE_INTENTS).toContain("ADD_EXPENSE");
    });

    it("should include UPDATE_STOCK as destructive", () => {
      expect(DESTRUCTIVE_INTENTS).toContain("UPDATE_STOCK");
    });

    it("should include CREATE_PAYMENT_LINK as destructive", () => {
      expect(DESTRUCTIVE_INTENTS).toContain("CREATE_PAYMENT_LINK");
    });

    it("should NOT include ASK_EXPENSE_SUMMARY as destructive", () => {
      expect(DESTRUCTIVE_INTENTS).not.toContain("ASK_EXPENSE_SUMMARY");
    });

    it("should NOT include ASK_LOW_STOCK as destructive", () => {
      expect(DESTRUCTIVE_INTENTS).not.toContain("ASK_LOW_STOCK");
    });
  });

  describe("Intent Enum", () => {
    it("should have at least 20 intents", () => {
      const intents = Object.values(CopilotIntentEnum.Values);
      expect(intents.length).toBeGreaterThanOrEqual(20);
    });

    it("should include all finance intents", () => {
      expect(CopilotIntentEnum.Values).toHaveProperty("ADD_EXPENSE");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_EXPENSE_SUMMARY");
      expect(CopilotIntentEnum.Values).toHaveProperty("CREATE_PAYMENT_LINK");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_COD_STATUS");
    });

    it("should include all inventory intents", () => {
      expect(CopilotIntentEnum.Values).toHaveProperty("UPDATE_STOCK");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_LOW_STOCK");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_TOP_MOVERS");
    });

    it("should include all ops intents", () => {
      expect(CopilotIntentEnum.Values).toHaveProperty("TAG_VIP");
      expect(CopilotIntentEnum.Values).toHaveProperty("REMOVE_VIP");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_HIGH_RISK");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_RECOVERED_CARTS");
    });

    it("should include all analytics intents", () => {
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_KPI");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_REVENUE");
      expect(CopilotIntentEnum.Values).toHaveProperty("ASK_ORDER_COUNT");
    });
  });
});
...
describe("Entitlement Gating Logic", () => {
  ...
  describe("Growth Plan (+ INVENTORY, PAYMENTS)", () => {
    ...
    it("should allow CREATE_PAYMENT_LINK", () => {
      expect(checkEntitlement("CREATE_PAYMENT_LINK", growthFeatures)).toBe(
        true,
      );
    });
...
describe("Confirmation Flow", () => {
  it("should require confirmation for destructive intents", () => {
    for (const intent of DESTRUCTIVE_INTENTS) {
      ...
      expect(DESTRUCTIVE_INTENTS).toContain(intent);
    }
  });

  it("should have at least 5 destructive intents", () => {
    expect(DESTRUCTIVE_INTENTS.length).toBeGreaterThanOrEqual(5);
  });
  ...
  it("should not mix query and action intents", () => {
    ...
    for (const intent of queryIntents) {
      expect(DESTRUCTIVE_INTENTS).not.toContain(intent);
    }
  });
});
