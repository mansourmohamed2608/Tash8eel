...
describe('Order Flows (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    // Clear token usage
    await pool.query('DELETE FROM merchant_token_usage WHERE merchant_id = $1', [merchantId]);
  }, 60000);
  ...
  describe('Happy Path - Order Flow', () => {
    ...
    it('3. Customer provides address', async () => {
      const response = await sendMessage(customer, 'عنواني 15 شارع التحرير المعادي القاهرة شقة 5');
  ...
  describe('Stress Tests - Complex/Broken Input', () => {
    ...
    it('should handle special characters', async () => {
      ...
      const response = await sendMessage(customer, '!@#$%^&*()_+-=[]{}|;:,.<>?');
  ...
  describe('Different Customer - New Conversation', () => {
    it('should create separate conversations for different customers', async () => {
      ...
      expect(response1.body.conversationId).not.toBe(response2.body.conversationId);
    }, 60000);
  ...
  describe('SQL Injection Attempt', () => {
    it('should safely handle SQL injection in text', async () => {
      ...
      const response = await sendMessage(customer, "'; DROP TABLE merchants; --");
    ...
    it('should safely handle SQL injection in merchantId', async () => {
      ...
      // Should return 404 (merchant not found) not crash
  ...
  describe('Angry Customer / Escalation', () => {
    it('should handle angry message professionally', async () => {
      ...
      const response = await sendMessage(customer, 'خدمتكم وحشة جداً ومحدش بيرد!');
    ...
    it('should handle complaint about order', async () => {
      ...
      const response = await sendMessage(customer, 'الطلب اتأخر كتير ومش راضي خالص');
    ...
    it('should handle threat to complain', async () => {
      ...
      const response = await sendMessage(customer, 'هعمل شكوى في حماية المستهلك');
  ...
  describe('Cart Operations', () => {
    it('should not duplicate items when mentioned again', async () => {
      ...
      // Confirm same item is not added again
      const items1 = r1.body.cart.items;
      const tshirtCount = items1.filter((i: any) => i.name.includes('تيشيرت')).length;
