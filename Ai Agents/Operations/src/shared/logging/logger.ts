...
import { getCorrelationId, getMerchantId } from '../middleware/correlation-id.middleware';
...
const PII_PATTERNS = [
  ...
  { pattern: /"phone"\s*:\s*"([^"]+)"/g, replacer: (match: string, phone: string) => `"phone":"${maskPhone(phone)}"` },
];
...
const maskPii = (obj: unknown): unknown => {
  if (typeof obj === 'string') {
    ...
    for (const { pattern, replacer } of PII_PATTERNS) {
      result = result.replace(pattern, replacer as (substring: string, ...args: unknown[]) => string);
    }
    return result;
  }
