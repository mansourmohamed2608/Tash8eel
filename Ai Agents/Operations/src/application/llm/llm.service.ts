...
import { withRetry, withTimeout, getTodayDate } from '../../shared/utils/helpers';
import {
...
export interface LlmResult {
  ...
  action?: ActionType;
  reply?: string;
  cartItems?: Array<{ name: string; quantity?: number; size?: string; color?: string }>;
  customerName?: string;
  phone?: string;
  address?: string;
...
export function createLlmResult(response: ValidatedLlmResponse, tokensUsed: number, llmUsed: boolean): LlmResult {
  // Filter products to only include those with names
  const products = response.extracted_entities?.products?.filter((p: any) => p.name) || [];
...
export class LlmService {
  ...
  constructor(
    ...
    @Inject(MERCHANT_REPOSITORY) private merchantRepository: IMerchantRepository,
  ) {
    this.client = new OpenAI({
      apiKey: this.configService.get<string>('OPENAI_API_KEY'),
    });
    this.model = this.configService.get<string>('OPENAI_MODEL', 'gpt-4o-mini');
    this.maxTokens = parseInt(this.configService.get<string>('OPENAI_MAX_TOKENS', '2048'), 10);
    this.timeoutMs = parseInt(this.configService.get<string>('OPENAI_TIMEOUT_MS', '30000'), 10);
  }
  ...
  async processMessage(context: LlmContext): Promise<LlmResult> {
    ...
    if (!budgetCheck.hasRemaining) {
      logger.warn('Token budget exceeded', { merchantId: merchant.id, remaining: budgetCheck.remaining });
    ...
    try {
      ...
      const parsedResponse = (response as any).choices?.[0]?.message?.parsed 
      ...
      await this.merchantRepository.incrementTokenUsage(merchant.id, getTodayDate(), tokensUsed);
  ...
  private async callOpenAI(
    ...
    return this.client.beta.chat.completions.parse({
      ...
      response_format: {
        type: 'json_schema',
        json_schema: LLM_RESPONSE_JSON_SCHEMA as OpenAI.ResponseFormatJSONSchema['json_schema'],
      },
  ...
  private validateResponse(parsed: unknown): ValidatedLlmResponse {
    ...
    if (!result.success) {
      logger.warn('LLM response validation failed', { errors: result.error.errors });
  ...
  private buildSystemPrompt(merchant: Merchant, catalogItems: CatalogItem[]): string {
...
`;
  ...
  private buildConversationHistory(messages: Message[]): OpenAI.ChatCompletionMessageParam[] {
  ...
  private buildUserPrompt(conversation: Conversation, customerMessage: string): string {
    ...
    if (info.address) {
      const addr = typeof info.address === 'object' ? info.address.raw_text || JSON.stringify(info.address) : info.address;
    ...
    if (!info.address || (typeof info.address === 'object' && !info.address.raw_text)) missingParts.push('العنوان الكامل');
...
تعليمات: تحليل الرسالة ورد بشكل مناسب. إذا كانت هناك معلومات ناقصة ولم يطلبها العميل بعد، اسأل عنها.`;
  ...
  private async checkTokenBudget(merchant: Merchant): Promise<{ hasRemaining: boolean; remaining: number }> {
    const usage = await this.merchantRepository.getTokenUsage(merchant.id, getTodayDate());
