...
export class MockDeliveryAdapter implements IDeliveryAdapter {
  ...
  private shipments = new Map<string, {
    trackingId: string;
    status: string;
    events: Array<{ timestamp: Date; status: string; description: string; location?: string }>;
  ...
  async getStatus(request: DeliveryStatusRequest): Promise<DeliveryStatusResponse> {
    logger.info('Mock: Getting delivery status', { trackingId: request.trackingId });
    ...
    const statusProgression = ['pending', 'picked_up', 'in_transit', 'out_for_delivery', 'delivered'];
  ...
  async cancelDelivery(trackingId: string): Promise<boolean> {
    ...
    if (shipment.status === 'delivered' || shipment.status === 'out_for_delivery') {
