...
import { MerchantCategory, ConversationState, ActionType, OrderStatus, MessageDirection } from '../../shared/constants/enums';
import { IDeliveryAdapter, DELIVERY_ADAPTER } from '../adapters/delivery-adapter.interface';
...
import { IShipmentRepository, SHIPMENT_REPOSITORY } from '../../domain/ports/shipment.repository';
import { ICustomerRepository, CUSTOMER_REPOSITORY } from '../../domain/ports/customer.repository';
import { ICatalogRepository, CATALOG_REPOSITORY } from '../../domain/ports/catalog.repository';
import { IKnownAreaRepository, KNOWN_AREA_REPOSITORY } from '../../domain/ports/known-area.repository';
...
export class InboxService {
  ...
  constructor(
    ...
    @Inject(KNOWN_AREA_REPOSITORY) private readonly knownAreaRepo: IKnownAreaRepository,
  ...
  async processMessage(params: InboxMessageParams): Promise<InboxResponse> {
    ...
    if (!merchant.isActive) {
      throw new BadRequestException(`Merchant ${params.merchantId} is not active`);
    }
    ...
    if (llmResponse.customerName) collectedInfo.customerName = llmResponse.customerName;
    if (llmResponse.phone) collectedInfo.phone = llmResponse.phone;
    if (llmResponse.address) collectedInfo.address = { raw_text: llmResponse.address };
  ...
  private async processLlmAction(
    ...
    let cart = conversation.cart || { items: [], total: 0, subtotal: 0, discount: 0, deliveryFee: 0 };
    ...
    if (llmResponse.deliveryFee && llmResponse.deliveryFee > 0) {
      ...
      const subtotal = cart.subtotal || cart.items.reduce((sum: number, item: any) => sum + item.total, 0);
      const discount = cart.discount || 0;
      cart.total = subtotal - discount + llmResponse.deliveryFee;
    }

    // Publish CartUpdated event if cart changed
    if (llmResponse.cartItems?.length || llmResponse.discountPercent || llmResponse.deliveryFee) {
      await this.outboxService.publishEvent({
  ...
  private async updateCart(
    ...
    merchantId: string,
  ): Promise<{ items: any[]; total: number; subtotal?: number; discount?: number; deliveryFee?: number }> {
    ...
    for (const newItem of newItems) {
      ...
      const existingIndex = items.findIndex((i: any) => i.productId === catalogItem.id);
    ...
    const filteredItems = items.filter((i: any) => i.quantity > 0 && i.unitPrice > 0);
    
    // Calculate subtotal (sum of all items)
    const subtotal = filteredItems.reduce((sum: number, item: any) => sum + item.total, 0);
  ...
  private async createOrder(
    ...
    if (llmResponse.customerName || llmResponse.address || llmResponse.phone) {
      const addressObj = llmResponse.address ? { raw_text: llmResponse.address } : undefined;
    ...
    const deliveryAddr = llmResponse.address ? { raw_text: llmResponse.address } : customer.address;
   ...
   */
  private determineNewState(action: ActionType, currentState: ConversationState): ConversationState {
    switch (action) {
      case ActionType.ORDER_CONFIRMED:
      case ActionType.CREATE_ORDER:
