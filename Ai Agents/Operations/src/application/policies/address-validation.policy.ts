...
abstract class BaseAddressValidator implements IAddressValidationPolicy {
  ...
  normalizeArea(
    ...
    const directMatch = knownAreas.find(
      (ka) => ka.areaNameAr === normalized || ka.areaNameEn?.toLowerCase() === normalized.toLowerCase(),
    );
    if (directMatch) {
    ...
    const aliasMatch = knownAreas.find((ka) =>
      ka.areaAliases.some(
        (alias) => alias.toLowerCase() === normalized.toLowerCase() || normalized.includes(alias),
      ),
    );
    if (aliasMatch) {
    ...
    const partialMatch = knownAreas.find(
      (ka) => normalized.includes(ka.areaNameAr) || ka.areaNameAr.includes(normalized),
    );
    if (partialMatch) {
...
export class AddressValidationPolicyFactory {
  ...
  getValidator(city?: string): IAddressValidationPolicy {
    ...
    return this.validators.get(city.toLowerCase()) || this.validators.get(city) || this.defaultValidator;
  }
}
