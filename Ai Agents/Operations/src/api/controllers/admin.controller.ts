import { 
  Controller, Get, Post, Param, Query, Body, Logger, UseGuards, Inject 
} from '@nestjs/common';
...
export class AdminController {
  ...
  async togglePromotion(
    ...
    @Body() body: { enabled: boolean; discountPercent?: number; description?: string },
  ): Promise<any> {
    ...
    const updatedRules = {
      ...
      activePromotion: {
        ...
        discountPercent: body.discountPercent || merchant.negotiationRules.activePromotion?.discountPercent || 10,
        description: body.description || merchant.negotiationRules.activePromotion?.description || 'عرض خاص',
      },
    };
      ...
      await this.pool.query(
        `INSERT INTO catalog_items (id, merchant_id, name_ar, base_price, category, sku, variants, is_available, created_at, updated_at)
         ...
         ON CONFLICT (merchant_id, sku) DO UPDATE SET
           ...
           updated_at = NOW()`,
        [uuidv4(), merchantId, item.name_ar, item.base_price, item.category, item.sku, JSON.stringify(item.variants || [])],
      );
    }
  }
