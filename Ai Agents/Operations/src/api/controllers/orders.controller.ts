import { 
  Controller, Get, Param, Query, Logger, Inject, NotFoundException, ForbiddenException
} from '@nestjs/common';
...
export class OrdersController {
  ...
  @ApiQuery({ name: 'merchantId', description: 'Merchant ID for tenant isolation' })
  ...
  @ApiQuery({ name: 'offset', description: 'Pagination offset', required: false })
  async listOrders(
    @Query('merchantId') merchantId: string,
    ...
    const result = paginated.map((order: Order) => this.mapOrderToDto(order, null));
  ...
  @ApiParam({ name: 'orderNumber', description: 'Order number (e.g., ORD-240115-ABC1)' })
  @ApiQuery({ name: 'merchantId', description: 'Merchant ID for tenant isolation' })
  ...
  private mapOrderToDto(order: Order, shipment: Shipment | null): OrderResponseDto {
    return {
      id: order.id,
