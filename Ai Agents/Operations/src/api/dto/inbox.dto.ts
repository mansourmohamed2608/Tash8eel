...
export class InboxResponseDto {
  ...
  @ApiProperty({ 
    ...
    enum: ['greet', 'update_cart', 'collect_slots', 'counter_offer', 'accept_negotiation', 'reject_negotiation', 'order_confirmed', 'track_order', 'escalate', 'fallback']
  })
  action: string;

  @ApiProperty({ 
    description: 'Current cart state',
    example: { items: [{ name: 'تيشيرت أبيض', quantity: 2, unitPrice: 150, total: 300 }], total: 300 }
  })
  cart: any;
