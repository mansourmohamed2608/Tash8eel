...
export class ConversationRepository implements IConversationRepository {
  ...
  async findByMerchantAndSender(merchantId: string, senderId: string): Promise<Conversation | null> {
    const result = await this.pool.query(
  ...
  private mapToEntity(row: Record<string, unknown>): Conversation {
    return {
      ...
      nextFollowupAt: row.next_followup_at ? new Date(row.next_followup_at as string) : undefined,
