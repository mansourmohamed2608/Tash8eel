...
export class MerchantRepository implements IMerchantRepository {
  ...
  async update(id: string, input: UpdateMerchantInput): Promise<Merchant | null> {
    ...
    if (input.negotiationRules !== undefined) {
      ...
      values.push(JSON.stringify({ ...existing.negotiationRules, ...input.negotiationRules }));
    }
  ...
  async getTokenUsage(merchantId: string, date: string): Promise<MerchantTokenUsage | null> {
    const result = await this.pool.query(
