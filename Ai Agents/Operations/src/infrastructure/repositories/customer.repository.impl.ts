...
export class CustomerRepository implements ICustomerRepository {
  ...
  async findByMerchantAndSender(merchantId: string, senderId: string): Promise<Customer | null> {
  ...
  async findByPhone(merchantId: string, phone: string): Promise<Customer | null> {
    const result = await this.pool.query(
  ...
  private mapToEntity(row: Record<string, unknown>): Customer {
    return {
      ...
      lastInteractionAt: row.last_interaction_at ? new Date(row.last_interaction_at as string) : undefined,
