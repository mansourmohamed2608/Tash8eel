...
async function seedMerchants(pool: Pool): Promise<void> {
  const merchants = [
    {
      id: 'demo-clothes',
      name: 'متجر الملابس',
  ...
  for (const merchant of merchants) {
    ...
    `, [
      ...
      merchant.name,
      merchant.category,
      `mk_${merchant.id.replace('-', '_')}_${uuidv4().slice(0, 8)}`,
      merchant.city,
...
async function seedCatalogItems(pool: Pool): Promise<void> {
  const catalogData = {
    'demo-clothes': [
    ...
    'demo-supermarket': [
      { name: 'لبن كامل الدسم 1 لتر', price: 28, category: 'ألبان', stock: 200 },
      ...
      { name: 'مياه معدنية 1.5 لتر', price: 8, category: 'مشروبات', stock: 300 },
    ...
    for (const item of items) {
      await pool.query(`
        ...
        ON CONFLICT (merchant_id, name) DO UPDATE SET
          ...
          updated_at = NOW()
      `, [uuidv4(), merchantId, item.name, item.price, item.category, item.stock]);
    }
